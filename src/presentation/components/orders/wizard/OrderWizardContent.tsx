...
export default function OrderWizardContent({ onSaved }: { onSaved: () => void }) {
  ...
  const currentFlowStep = primaryDevice ? getFlowStep(primaryDevice.id) : 1;
  const currentMainStep =
    currentFlowStep === 1 ? 1 : currentFlowStep === 2 ? 2 : currentFlowStep === 3 ? 3 : 1;
  ...
  return (
    <Fragment>
      <div className="rounded-3xl bg-gradient-to-br from-[#08153c] via-[#1a2668] to-[#3b4fb4] p-4 shadow-[0_30px_80px_-35px_rgba(15,23,42,0.8)] md:p-6">
        <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="hidden rounded-2xl border border-white/10 bg-white/5 p-6 text-white backdrop-blur md:block">
            ...
            <h2 className="text-5xl leading-[0.95] font-bold">
              Registro de <span className="text-violet-300">Dispositivo</span>
            </h2>
            <p className="mt-4 text-lg text-white/85">En 1 minuto. Simple, moderno y poderoso.</p>
            <ul className="mt-6 space-y-3 text-sm text-white/85">
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-violet-300" /> Proceso guiado en 5 pasos
              </li>
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-violet-300" /> Validaciones inteligentes
              </li>
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-violet-300" /> Checklist inmutable y rápido
              </li>
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-violet-300" /> Diseño moderno y responsivo
              </li>
            </ul>
          </aside>

          <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-2xl font-bold text-slate-900">Nueva Orden de Trabajo</h2>
              <div className="flex items-center gap-2 text-xs md:text-sm">
                {[
                  { step: 1, label: "Checklist", icon: NotebookPen },
                  { step: 2, label: "Desbloqueo", icon: Lock },
                  { step: 3, label: "Servicios", icon: Wrench },
                  { step: 4, label: "Resumen", icon: Check },
                  { step: 5, label: "Confirmación", icon: CircleHelp },
                ].map((item, idx) => {
                  const done = currentMainStep > item.step;
                  const active = currentMainStep === item.step;
                  ...
                  return (
                    <Fragment key={item.step}>
                      ...
                      {idx < 4 && <span className="h-px w-4 bg-slate-200" />}
                    </Fragment>
                  );
                })}
              </div>
                            ...
                            <div className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-zinc-50 p-4 shadow-sm">
                              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-zinc-200 bg-white">
                                <img
                                  src={
                                    cardImage ||
                                    "https://dummyimage.com/100x100/e2e8f0/475569&text=?"
                                  }
                      ...
                      {serialFieldOpenByDevice[device.id] || !!device.deviceSerial ? (
                        <div className="flex gap-2">
                          <input
                            ...
                            onChange={(e) =>
                              updateDevice(device.id, { deviceSerial: e.target.value })
                            }
                              ...
                              onChange={(e) => {
                                ...
                                if (type === "pattern") {
                                  ...
                                  updateDevice(device.id, {
                                    ...
                                    deviceUnlockPattern: [],
                                    deviceUnlockCode:
                                      type === "none" ? "" : device.deviceUnlockCode,
                                  });
                                }
                              }}
                            >
                            ...
                            <ul className="list-disc pl-5 text-slate-600">
                              {device.selectedServices.map((service) => (
                                <li key={`${device.id}-detail-${service.id}`}>
                                  {service.name} —{" "}
                                  {formatCLP(device.servicePrices[service.id] || 0)}
                                </li>
                              ))}
                            </ul>
                          ) : (
            ...
            <div className="flex justify-center">
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-3 font-medium text-white hover:brightness-110"
              >
                ➕ Agregar Otro Equipo
              </button>
            </div>
            ...
            {/* Prioridad y Fechas */}
            <div className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-slate-50/60 p-4 md:grid-cols-3">
              <div>
            ...
            <div className="flex justify-end gap-4 border-t border-slate-100 pt-4">
              ...
              <button
                ...
                disabled={
                  ...
                  isSubmitting ||
                  devices.some(
                    (device) => device.problemDescription.length > MAX_DESCRIPTION_LENGTH,
                  )
                }
                className="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-2 text-white hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              >
